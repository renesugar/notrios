...
| Native archive v2/backup | Semantic export/verify/restore implemented; physical full-snapshot default reopened in v0.7 G14a-G14e | archive service | Schema-v12 database/replica identity, manifest-last SHA-256 objects, typed bounded records, explicit restore intent, and strict verification are live. P3a made inventory bounded; P3b added opt-in `objects.pack.v1`; P4 proved both layouts with attachments/source bundles. G14 catch-up still exports loose objects and stores one ZIP entry per object, adding about 25% at 100/500 notes. Before G15, G14a-G14e compare packed archive-v2 and consistent SQLite-image + packed-assets candidates against restic/borg at the supplied full scale, implement/integrate the measured choice, and freeze it without removing semantic subset/merge compatibility. |
...
| Snapshot catch-up/reset | Implemented G10/G14; scalability remediation G14a-G14e blocks later sync work | archive/sync | Signed request, permissioned/resumable encrypted archive-v2 snapshot, explicit restore intent, catch-up floor, then incremental replay are live. The current loose-object ZIP wrapper has not passed a full-corpus scale gate; G14a-G14e select and integrate a bounded physical representation before scheduling/UI/retention. |
