...
| Native archive v2/backup | Semantic export/verify/restore implemented; physical full-snapshot default reopened in v0.7 G14a-G14e | archive service | Schema-v12 database/replica identity, manifest-last SHA-256 objects, typed bounded records, explicit restore intent, and strict verification are live. P3a made inventory bounded; P3b added opt-in `objects.pack.v1`; P4 proved both layouts with attachments/source bundles. G14 catch-up still exports loose objects and stores one ZIP entry per object. G14a's generated 100k calibration measured 100,093 loose files, 11.15% stored-ZIP overhead, 6,004 bytes of framing overhead, and a 797,937,664-byte incremental-replay RSS failure. G14b-G14e now compare packed archive-v2 and consistent SQLite-image + packed-assets candidates against restic/borg at the supplied full scale, implement/integrate the measured choice, and freeze it without removing semantic subset/merge compatibility. |
...
| Snapshot catch-up/reset | Implemented G10/G14; scalability remediation G14a-G14e blocks later sync work | archive/sync | Signed request, permissioned/resumable encrypted archive-v2 snapshot, explicit restore intent, catch-up floor, then incremental replay are live. G14a's harness/calibration is complete and records the loose baseline's file-count and incremental-memory failures. G14b-G14e select and integrate a bounded physical representation before scheduling/UI/retention. |
