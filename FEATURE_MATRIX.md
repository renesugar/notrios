...
| Native archive v2/backup | Semantic export/verify/restore implemented; physical full-snapshot option B selected, implementation planned G14c-G14e | archive service | Schema-v12 database/replica identity, manifest-last SHA-256 objects, typed bounded records, explicit restore intent, and strict verification are live. P3a made inventory bounded; P3b added opt-in `objects.pack.v1`; P4 proved both layouts with attachments/source bundles. G14 catch-up still exports loose objects and stores one ZIP entry per object. G14a measured the loose-path shape and replay-memory failures. G14b's full-corpus matrix selected a required same-schema SQLite image plus bounded packed assets for whole-library backup/catch-up (2.33x faster locally than semantic restore), while retaining packed semantic archive-v2 for subset/merge/interchange/fallback. G14c-G14e implement, integrate, and freeze that measured choice. |
...
| REST sync data plane | Implemented v0.7 G14 | `internal/syncrest`, `internal/syncbackup`, httpapi | The same artifacts over the authenticated surface, by implementing G11's `Carrier` interface — one protocol, two couriers, transcript parity by construction and +0.07% overhead at 500 notes. Publishing takes no namespace: the peer derives it from the authenticated principal. Snapshots are addressed by opaque id, produced only for an explicitly permitted replica, sealed in authenticated frames, resumable from the local file's own length, and verified by archive-v2 before any restore. No scheduling or retries (G15); `rclone` remains a conformance carrier only and target `none` stays the default. |
...
| Snapshot catch-up/reset | Implemented G10/G14; scalability remediation G14c-G14e blocks later sync work | archive/sync | Signed request, permissioned/resumable encrypted archive-v2 snapshot, explicit restore intent, catch-up floor, then incremental replay are live. G14a recorded the loose baseline's file-count and incremental-memory failures; G14b selected the compatible SQLite-image plus bounded packed-assets representation while retaining semantic archive-v2. G14c-G14e implement, integrate, and freeze it before scheduling/UI/retention. |
