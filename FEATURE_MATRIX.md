...
| Offline frontend assets | **Defect — planned E6a** | web UI | Math (KaTeX) and code highlighting are fetched from `unpkg.com` at runtime; with the CDN unreachable a formula silently renders as raw LaTeX. One note load issues 13 third-party requests. Inconsistent with the local-first premise and with the media policy that quarantines remote *images* while remote *scripts* load unchecked. |
| Math rendering (KaTeX) | Implemented, online only | web UI | `$…$` and `$$…$$` render. Blocked by the same CDN dependency as above; E6a self-hosts it. FTS indexes the LaTeX source, not the rendered output. |
| HTML paste to Markdown | Planned E6b | web UI | A pasted HTML table renders but stays raw HTML in the note source, so blocks, link extraction, and portable export cannot see into it. Simple tables convert on paste; anything with spans or nested blocks pastes unchanged. |
