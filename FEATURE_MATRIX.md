...
| Native archive v2/backup | Implemented; compatible physical whole-library default frozen G14e | archive service | Semantic archive-v2 remains the portable subset/merge/fork/publication/incompatible-schema contract with loose and packed readers. G14b selected, G14c implemented, G14d integrated, and G14e full-scale accepted `sqlite-image+packed-assets.v1` for compatible whole-library backup/catch-up through deterministic USTAR and NBK1, strict admission, emergency replacement, and post-vector replay. It is a distinct required capability, not archive-v3. |
...
## Documentation integrity and delivery
...
| Source-adjacent documentation anchors and truth grades | Implemented v0.7 G18a/G18c; advisory review planned G18f | documentation tooling | `docaudit` resolves Go and TS/TSX declarations, typed claims, Markdown slots, executable fences, and GUI owners. Its checked 351-unit baseline reports 0 executed, 8 generated, 4 claimed, and 339 unverified; deterministic graph/report freshness gates CI while future calibrated blind-explanation review remains advisory. |
| Executed documentation examples and GUI journeys | Planned v0.7 G18d/G18e | CLI/API/UI tests | CLI/config/REST/MCP examples require semantic postconditions, not just success codes. GUI procedures run at desktop/mobile sizes and report action length/branching as review evidence, not an automatic usability score. |
| Hugo/Ledger documentation site | Planned v0.7 G18b/G18g | docs site | Investigate then migrate the current bespoke Marked/Pagefind build to pinned Apache-2.0 `hugo-theme-ledger`, preserving `/notrios/` links, static Pagefind, offline assets, raw Markdown Help seeding, clean release-ZIP builds, and accessibility. No Bluge service is planned for the small docs corpus. |
...
| Sync MCP control plane | Implemented v0.7 G15 | MCP adapter | `mcp.sync_scope` defaults disabled; explicit status/control permits bounded status/conflict inspection and path-free ordinary sync/resource jobs only. No keys, backups, restore, enrollment, retirement, purge, catch-up, reset, or bulk bytes in context. |
...
| Native record-level sync | Implemented through safe retention (v0.7 G17) | sync service/UI | Transactional change log, contiguous state vectors, deterministic merge, encrypted directory/REST carriers, lazy resources, physical catch-up, schema-v26 durable jobs, Sync Center, and schema-v27 signed retirement plus acknowledgement/snapshot-gated retention are live. G17a-G17b completed the evidence-preservation contract and immutable reserve; G18 completed the portability handoff. G18a-G18g documentation integrity/site work, G19 compatibility, and G20 release acceptance remain. |
| Shared-core/C-ABI portability handoff | Contract complete v0.7 G18; implementation planned v0.8 | service/application architecture | Audited 109 API operations and 19 platform capabilities; froze ABI-major-1 opaque-handle, explicit-buffer, typed-error, cancel/poll/event, and bounded-stream contract. Linux build modes are feasible, while Android cgo stops at the unresolved SQLite linkage boundary. No ABI, Flutter app, Android build, or mobile support is claimed. |
...
| REST sync data plane | Implemented v0.7 G14/G15 | `internal/syncrest`, `internal/syncbackup`, `internal/syncjobs`, httpapi | G14 carries the same artifacts over authenticated REST; G15 adds explicit durable jobs, per-target leases/byte budgets, checkpoint-safe cancellation, and jittered retry. The worker drains queued/due rows but never schedules periodic sync. Local REST controls the full safe job surface; MCP is separately scoped and path-free. |
| MCP sync control | Implemented v0.7 G15 | `internal/syncjobs`, httpapi | `mcp.sync_scope` defaults to `disabled`; `status` exposes bounded content-free status/conflicts and `control` adds plan/start/resource-fetch plus retry/cancel of MCP-owned ordinary jobs. No paths, URLs, credentials, keys, artifact bytes, enrollment, backup/restore, retirement, purge, catch-up/restore-prep control, or reset. |
| Sync, conflict, and recovery UI | Implemented v0.7 G16 | React, Wails bridge, local httpapi | Responsive Sync Center names active identity, configures none/directory/REST, uses a native-only folder chooser, pairs explicitly, separately grants snapshot permission, shows jobs/peers/resources/repairs, resolves three-way conflicts with a two-parent revision, stages catch-up, and creates/reviews NPB1 password backups without retaining passwords or applying restore. Mobile-sized design is validated; no mobile build is claimed. |
...
| Snapshot catch-up/reset | Implemented and full-scale accepted through G14e | archive/sync | Signed request, explicit permission, resumable encrypted compatible physical snapshot, strict admission, emergency restore, fresh replica/floor, then scoped incremental replay are live over REST and directory couriers. Semantic archive-v2 remains the portable fallback; G15's schema can represent local catch-up/restore-prep jobs but MCP cannot start or control them. |
