...
| Portable Markdown vault export | Contract published; external consumer absent | movenotes-v3 | G19 publishes strict archive-v2 schemas, registry, goldens, and current/previous compatibility probes. The local MoveNotes audit found no `notrios2sql.py` consumer, so no external integration is claimed. |
...
| Native record-level sync | Implemented and release-accepted v0.7 | sync service/UI | Transactional change log, contiguous state vectors, deterministic merge, encrypted directory/REST carriers, lazy resources, physical catch-up, schema-v26 durable jobs, Sync Center, and schema-v27 signed retirement plus acknowledgement/snapshot-gated retention are live. G17a-G17b completed evidence custody, G18 the portability/docs contracts, G19 archive compatibility, and G20 full-system/security acceptance. |
