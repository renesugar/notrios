...
| Native record-level sync | Implemented through safe retention (v0.7 G17) | sync service/UI | Transactional change log, contiguous state vectors, deterministic merge, encrypted directory/REST carriers, lazy resources, physical catch-up, schema-v26 durable jobs, Sync Center, and schema-v27 signed retirement plus acknowledgement/snapshot-gated retention are live. G17a completed the evidence-preservation contract; G17b's scope, exact signer, and provider order are resolved, but operationally unapproved G17b must backfill the curated manifest/signatures/checkpoint timestamp and external ISO reserve before any push. G18 then owns the portability handoff, followed by G18a-G18g documentation integrity/site work, G19 compatibility, and G20 release acceptance. |
