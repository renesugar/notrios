...
| Move a note between notebooks | Partial | document service / UI | Store (`MoveDocumentToNotebook`), REST (`POST /api/v1/documents/{id}/notebook`), and MCP (`move_note_to_notebook`) are implemented. **The GUI and `notriosctl` have no move affordance at all**, and the GUI creates every note in the default "Notes" notebook because its `createDocument` sends no `notebook_id` — so a note filed wrongly cannot be corrected from the built-in client. Planned as v0.6 F0; multi-note move is F1. |
...
| Note templates and task extraction | Deferred to v0.6 | document service | Listed under v0.5 in `ROADMAP.md` but never entered `PLAN.md` as a task; v0.5 shipped thirteen slices without it. Recorded here rather than left as a silent gap. Nothing in v0.5 depends on it. |
| Graph visualization (a graph *view*) | Deferred to v0.6 | UI | v0.5 E4 shipped the traversal, shortest-path, and orphan/hub **data** and named itself "traversal, paths, and visualization data" for that reason. No graph canvas exists in the GUI; the roadmap bullet said "visualization and path finding" and only the second half landed. |
