...
| Native record-level sync | Implemented through durable operation control (v0.7 G15); lifecycle/UI/retention remain | sync service | Transactional change log, contiguous state vectors, deterministic merge, encrypted directory/REST carriers, lazy resources, physical catch-up, and schema-v26 durable per-target jobs with bounded retry/cancellation are live. G16-G20 own UI, expiry/retention, search-derived repair, notebooks, and release acceptance. |
