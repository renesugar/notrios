...
| Ephemeral shared-directory transport | Implemented v0.7 G11 | `internal/synccarrier`, CLI | Two replicas exchange encrypted signed artifacts through a disposable folder and converge; `notriosctl sync init/bundle/pair/status/discover/once` runs it. Every path segment below `notrios-sync/v1/` is a keyed blind, each replica writes only its own namespace, artifacts are named by what they logically are, and a publisher repairs its own unreadable copy. Discovery reports candidates and never enrols. No watcher or scheduler (G15), no cloud/removable-media evidence (G12), no snapshot transfer (G14). |
| REST sync transport | Planned v0.7 | sync service | The same object/envelope protocol over an authenticated API; G13 owns the security foundation and G14 the data plane. `rclone` remains a conformance carrier only, and target `none` stays the default. |
| Sync key material | Development provider v0.7 G11 | `internal/synckeys` | A `0600` JSON file holding one group key per epoch, this replica's Ed25519 signing key, and paired peers' public keys, refusing to open if other users can read it. Explicitly not an OS keychain: v0.8 selects the platform store, and pairing is a clear-text bundle until G13's short-lived exchange replaces it. |
