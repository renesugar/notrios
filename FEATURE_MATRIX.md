...
| Native record-level sync | Implemented through local recovery UI (v0.7 G16); lifecycle/retention remain | sync service/UI | Transactional change log, contiguous state vectors, deterministic merge, encrypted directory/REST carriers, lazy resources, physical catch-up, schema-v26 durable jobs, and the loopback/native Sync Center are live. G17-G20 own expiry/retention, search-derived repair, notebooks, and release acceptance. |
