...
Status: **Complete. Planned from `ROADMAP.md` after v0.7 G20 completed on
2026-08-31 and closed on 2026-09-09. Product version is 0.8.0 and the canonical
database schema remains v27** -- no v0.8 item added a migration, and the latest
is still `0027_sync_retention.sql` from v0.7. The GitHub push, the
`develop`-to-`main` pull request and the branch synchronization open v0.9; v0.8
made no external write.**
...
**35 items: 33 complete, 0 in progress, 0 not started, 2 deferred.**
...
| H13. v0.8 release wrap-up | complete | 2/2 | — |
...
## H13. v0.8 release wrap-up — complete
...
- **Internal artifact custody — Taken as the default, 2026-09-09.** The
  verified source snapshot and the unsigned Ubuntu package are in the local
  evidence directory and nowhere else. No GitHub Release, workflow artifact,
  reserve/ISO/media write, or installer publication.

**Outcome (2026-09-09).** v0.8 is closed. Product **0.8.0**, schema **v27
unchanged** -- the decision this item carried said to change the schema only for
a migration an approved item actually required, and none did: the newest
migration is still `0027_sync_retention.sql` from v0.7. The source snapshot is
`dist/notrios-v0.8-h13-030ef2d.zip`, 24,943,251 bytes and 2,320 entries, copied
byte-identically to the evidence directory with the unsigned `0.8.0-1` package
beside it. No push, no tag, no release.

**Packaging found four stale claims that `make validate` could not, and the
pattern is the finding.** Every one of them lived behind a gate that runs only
when you package:

*A dangling Go anchor, stale since H15.* `performance/v0.7-g18e` still named
`(*NativeUIBridge).ChooseSyncDirectory`, a method H15 renamed to
`ChooseDirectory(purpose)` a month ago. Nothing noticed because `docjourney`
runs from `package_release.sh` and not from `make validate`.

*The published site had been pinned at fifteen documents since v0.7*, while v0.8
added the features page and both journey catalogues. So were the route list, the
section count and the Pagefind page count -- four numbers describing a site that
had grown underneath them.

*The interface journeys would have shipped with every image broken.* The site
build never copied `docs/images/`, because until H15 there were none to copy.

*A dev-only advisory blocked the packager*, which is the gate working: `vitest`
carried GHSA-82fw-gwwq-j7x9 in `@vitest/mocker`. Upgraded to 4.1.11, 292 tests
still pass, and the dependency-licence inventory was re-taken rather than
re-hashed blindly.

**So the gates that need no browser moved into `make validate`** -- g18e, g18f,
g19 and g20. A gate that only runs at release time tells you at the worst
possible moment, and it told us four times in one afternoon. g18g stays out
because it needs Hugo and Pagefind from `docs-site/node_modules`.

**Two evidence records were re-derived from the present and are not any more.**
G20's release record read `internal/version/version.go` and required `0.7.0`,
so a finished milestone's evidence started failing on a fact about v0.8; it
checks that the version has not gone *backwards* now, which is what would
actually mean the tree is not the one it describes. Its own drift test moved
with it, from "any version but 0.7.0" to "a version older than 0.7.0".

**And one document restated the version by hand.** `docs/cli.md` said the
version was "currently `0.7.0`", written outside a generated block, in the
documentation for the command that prints the version. It is a gate now:
`TestNoDocumentRestatesTheProductVersion` fails any page under `docs/` that
claims to be current about it, which was broken on purpose to check.

**A slug disagreement I could not explain, handled by removing the ambiguity
rather than by guessing.** Hugo renders `## collections list / collections show`
as `collections-list--collections-show` and the section inventory as
`collections-list-...` with one dash -- but the older
`## snapshot create / snapshot verify / snapshot restore`, identical in form and
in the same file, gets single dashes from both. I could not account for the
difference and did not invent a rule to cover it. The three headings v0.8 added
now read "collections list **and** collections show" and so on, which both
slugifiers agree on and which reads better anyway. The unexplained case is
recorded here rather than papered over.

**The flaky gate is less flaky and more honest.**
`TestRuntimeProfilesStartTwoIsolatedDaemons` failed twice under the full suite
and passed six times alone, always on the second daemon: two daemons opening
databases and running migrations under a loaded machine take longer than the
ten-second deadline it had. Thirty seconds now, and a failed wait says whether
the process died and prints what it said, because "connection refused" reads the
same for a slow start and a crash and sends a reader to the wrong place.

**What v0.8 does not claim** is in `RELEASE_CHECKLIST.md` beside what it does,
and each entry has a gate behind it where one can exist: no Windows or macOS
installer, no Android product, no Wails v3 migration, no tag, no release, no
signing.
