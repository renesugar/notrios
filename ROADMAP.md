...
- **Putting a file in from a terminal.** The command line reads attachments and
  ...
  about the placement and not about the bytes: the product already separates the
  resource, the reference to it, and the link in the body. A command that adds
  the first two and prints the URI for the third guesses at nothing, and turns
  the one feature this milestone wrote off as a boundary back into a gap worth
  closing.
- **A command line that can read what it finds.** Searching returns note ids,
  ...
  the guide now shows with pipelines that were run rather than written. Only
  `doctor` still needs the machine form it never had. And fail a build when a
  command's described flags and its accepted flags disagree, which is the gap
  that produced a documented `sync retire --key` for a command taking `--peer`.
...
- **Python that leaves nothing behind, and a derived evidence index.** Running
  the repository's own scripts writes `__pycache__` directories across the tree
  — 19 of them, 48 files, all git-ignored and all regenerated by every validate
  run. `make clean` removes them, so they accumulate between cleans.

  Measured rather than assumed: deleting every one of them breaks nothing, and
  the sharper worry about *stale* bytecode does not apply to `__pycache__` in
  Python 3 — an orphaned `.pyc` whose source is gone is not importable (PEP
  3147) and a changed source invalidates its cache. The reason to stop writing
  them is the one that survives inspection: nothing here benefits from the
  cache, and `PYTHONDONTWRITEBYTECODE=1` is simpler than cleaning up after it.
  `PYTHONUNBUFFERED=1` belongs beside it because these scripts print progress a
  person reads while waiting, and buffered output through a pipe makes a long
  run look hung.
- **A Validation section that is three things, not one paragraph.** The README's
  runs together which arguments the profile scripts accept, where committed
  reference evidence lives, and what to read before tagging — and the middle
  one has drifted the same way the project status had: **77 `performance/`
  directories exist and 13 are named**, covering v0.3 through v0.5 while v0.7,
...
`PLAN.md` holds the active plan derived from this roadmap: 21 items, 12 complete, 2 in progress, 7 not started, 0 deferred.

Started and unfinished: J20, J21. What remains in each is in the plan's own Progress section.
