...
func TestSchemaV7MediaPolicyTables(t *testing.T) {
	...
	if status.SchemaVersion != CurrentSchemaVersion {
		t.Fatalf("schema version after re-bootstrap = %d, want %d", status.SchemaVersion, CurrentSchemaVersion)
	}
}
...
func TestSchemaUpgradeFromV6(t *testing.T) {
	...
	if status.SchemaVersion != CurrentSchemaVersion {
		t.Fatalf("upgraded schema version = %d, want %d", status.SchemaVersion, CurrentSchemaVersion)
	}
