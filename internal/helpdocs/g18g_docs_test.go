...
func TestG18gRepositoryDocsAreSeededByteForByte(t *testing.T) {
	...
	sort.Strings(paths)
	if len(paths) != 15 {
		t.Fatalf("expected exactly 15 repository Markdown files, got %d (%v)", len(paths), paths)
	}
	...
	if first.FilesSeen != 15 || first.NotesCreated != 15 || first.NotesUpdated != 0 || first.NotesRemoved != 0 {
	...
	if len(page.Documents) != 15 {
		t.Fatalf("expected 15 Help notes, got %d", len(page.Documents))
	}
	...
	if second.FilesSeen != 15 || second.NotesKept != 15 || second.NotesCreated != 0 || second.NotesUpdated != 0 || second.NotesRemoved != 0 {
