...
`notriosctl import twitter [--config path] [--db path] [--asset-store path] [--collection id] [--notebook Twitter] [--dry-run] <twitter-archive.zip | extracted-archive-dir>` imports a Twitter/X archive, read in place from the downloaded ZIP or from its extracted folder. Posts come from `tweets.js` and every `tweets-partN.js` in part order, plus `community-tweet.js`. Deleted posts are counted and not imported, and the post count is checked against `tweet-headers.js`. Tweets become Markdown notes in a "Twitter" notebook with expanded URLs and embedded media resources; hashtags become tags; provenance rows record author/@handle/thread/reply-to/post URL so conversation threads are recoverable; trashed tweets are never resurrected by re-imports.

`notriosctl import chatgpt [--options] <export.zip|export-dir|conversations.json>` and `notriosctl import claude [--options] <export.zip|export-dir|conversations.json>` import conversation exports as downloaded. ChatGPT accepts both shapes OpenAI ships: the export from ChatGPT itself, and the OpenAI Privacy Portal export, whose conversations and file library are nested ZIPs read in place; Claude accepts its export ZIP and the batch ZIPs one export is split into. Every conversations shard is read. Each conversation becomes one Markdown note (messages as role/timestamp sections; ChatGPT follows the current-node main path and abandoned branches are skipped) in a "ChatGPT"/"Claude" notebook, with provenance rows using the conversation ID as the thread ID. Code and execution output are fenced code blocks and browsing results are text, while thinking, reasoning recaps and tool plumbing are counted rather than imported. ChatGPT assets are imported as content-addressed resources and embedded, matched under either the file- or file_ id prefix, with Privacy Portal .dat names recovered from conversation_asset_file_names.json, then library_files.json, then the bytes themselves; its file library is imported too, with files no conversation references given notes of their own in a "ChatGPT Files" notebook. Claude attachments keep their extracted text under the file's name, files it only names are marked as absent, and each project becomes a note. The same trashed-note and idempotency rules apply.
