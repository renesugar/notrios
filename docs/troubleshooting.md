...
| `the pre-migration backup could not be made` | there is not enough room beside the database, or its directory is not writable. **The database was not migrated and is exactly as it was** — free some space, or fix the permissions on `data/pre-migration-backups/`, and open it again ([schema migrations](service.md#schema-migrations-and-their-backup)) |
| `another process is migrating this database` | a second Notrios started while the first was upgrading the schema. Wait for it to finish and try again; the lock exists so two processes cannot migrate at once |
| `the schema migration failed` | start Notrios again: it restores the verified copy automatically before opening anything, then stops and says so. The error already names the copy ([schema migrations](service.md#schema-migrations-and-their-backup)) |
| `an incomplete schema migration was rolled back` | not a failure of this run — a previous migration did not finish and your database has been put back as it was. Nothing was lost. If the upgrade keeps failing, move your notes across instead: `notriosctl export archive-v2` from the old library, then `notriosctl import archive` into a new one |
| `an incomplete schema migration could not be rolled back` | the backup no longer matches what was recorded when it was taken, so nothing was changed and both copies are left in place. Recover from your own backup, or use `notriosctl export archive-v2` if the partly migrated database still opens elsewhere |
